...
* Delete list of branches that come from a text file. Ensure that it's a plain text file with 1 branch per line and no trailing empty line. Let's say the text file is `branches.txt` and that it's stored in your home dir:
+
----
git push origin --delete $(cat ~/branches.txt)
----
...
----
+
or when you use the same `branches.txt` file from the previous bullet point:
+
----
git branch -D $(cat ~/branches.txt)
----

* When creating a new ssh key, run the following steps:
+
----
1. Load new key in ssh agent:
1a. eval "$(ssh-agent -s)"
1b ssh-add ~/.ssh/<id_ed25519> #path to your new ssh key

2. Add the public key to the git service (github, gitlab, bitbucket,...)

3. Test the connection: ssh -T git@gitlab.com

You should see something like "Welcome to Gitlab, @<your username)" or "Hi @<your username>! You've successfully authenticated..."
----