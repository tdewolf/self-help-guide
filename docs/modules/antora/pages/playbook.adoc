...
You must have installed the package `@antora/pdf-extension`.

In your playbook, enable this extension and set the attribute.
...
Then run the following command the build the PDF of the playbook:

----
npx --package @antora/pdf-extension antora -r @antora/pdf-extension virtual-appliance/antora-playbook-local.yml
----